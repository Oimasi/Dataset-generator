...
4.  Ключи в JSON должны быть **короткими и на английском языке**. Например, "Название компании" -> `company_name`, "Количество пассажиров" -> `passenger_count`. **Транслитерация русских слов запрещена!** Вместо `kolichestvo_pasazhirov` используй `passenger_count`. Ключи должны быть в стиле `snake_case`.
