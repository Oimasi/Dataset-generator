...
4.  Ключи в JSON должны быть **переведены на английский язык**. **Транслитерация запрещена**. Например, для "название компании" используй `company_name`, а не `nazvanie_kompanii`. Ключи должны быть в стиле lower_case_with_underscores и состоять не более чем из 3 слов.
